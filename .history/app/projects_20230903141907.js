               ...
               > */}
                  <View style={styles.container} >
                     <View style={styles.box}>
                        <Image style={{ top: 35, width: 250, height: 250, resizeMode: "contain", borderRadius: 10, }} source={{uri: item.image}} />
                        <Text style={styles.title}>{item.title}</Text>
                        <View style={{top: 50, gap: 10, flexDirection: "row"}}>
                           {item.icons.map((icon, index) => (
                              <Image key={index} source={{ uri: icon}} style={{ marginTop: 0, width: 25, height: 25}} />
                           ))}
                        </View>
