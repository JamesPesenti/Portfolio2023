               ...
               > */}
                  <View style={styles.container} >
                     <View style={styles.box}>
                        <WebView style={{ top: 35, width: 250, height: 250, resizeMode: "contain", borderRadius: 10, }} source={{html: item.image}} />
                        <Text style={styles.title}>{item.title}</Text>
                        <View style={{top: 0, gap: 10, flexDirection: "row"}}>
                           {item.icons.map((icon, index) => (
                              <Image key={index} source={{ uri: icon}} style={{ marginVertical: 10, width: 25, height: 25, borderRadius: 50}} />
                           ))}
                        </View>
